...
class ImageHelper(Helper):
    ...
    def get_test(self):
        test_set = torch.utils.data.Subset(self.test_dataset,range(0,len(self.test_dataset),8))
        print('len',len(test_set))
        test_loader = torch.utils.data.DataLoader(test_set,batch_size=self.params['test_batch_size'],shuffle=True)
        return test_loader
